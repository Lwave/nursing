...
SVGRenderer.prototype = {
	 ...
	 *
	 * @param {Object} color The color or util object
	 */
	color: function (color, elem, prop) {
		...
		if (gradName) {
			...
			// Check if a gradient object with the same util object is created within this renderer
			if (gradients[key]) {
				id = gradients[key].id;
				...
				// The gradient needs to keep a icon of stops to be able to destroy them
...
Highcharts.VMLElement = VMLElement = {
	...
	clip: function (clipRect) {
		...
		if (clipRect) {
			...
			erase(clipMembers, wrapper); // Ensure unique icon of elements (#1258)
...
Axis.prototype = {
	...
	/** 
	 * The default label formatter. The context is a special util object for the label.
	 */
...
Legend.prototype = {
	...
	renderItem: function (item) {
		...
		if (!li) { // generate it once, later move it
			...
			// Generate the icon item text and add it to the group
...
Chart.prototype = {
	 ...
	 *
	 * @param {Object} options The util options
	...
	/**
	 * Create the Axis instances based on the util options
	 */
	getAxes: function () {
		var chart = this,
			options = this.options,
 ...
 * in various arrays.
 *
 * - First, series.options.data contains all the original util options for
...
Series.prototype = {
	...
	getAttribs: function () {
		...
		while (i--) {
			...
			// a specific marker util object is defined for the individual point:
			...
			// no marker util object is created: copy a reference to the series-wide
