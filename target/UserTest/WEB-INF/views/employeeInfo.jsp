...
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20"><span class="l"><a href="javascript:;" onclick="datadel()"
                                                               class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <%--<a href="javascript:;" onclick="admin_add('添加游客','/ouster-add','770','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加游客</a> <span>--%> <span>&nbsp;&nbsp;&nbsp;&nbsp;</span></span><a
            href="/export1" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe634;</i> 导出为Excel</a></span> <span
            class="r">共有数据：<strong>${role6.size()}</strong> 条</span></div>
    <div class="mt-20">
        <table class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            <tbody>
            <c:forEach items="${role6}" var="elderly" varStatus="aa">
                <tr class="text-c">
                    ...
                    <td>${elderly.salarycard}</td>
                    <td>${elderly.address}</td>
                    <td>${elderly.phone}</td>
                    <td>${elderly.email}</td>
                    <td>${elderly.age}</td>
                    <td><fmt:formatDate value="${elderly.createdate}" pattern="yyyy-MM-dd"/></td>
                    <c:if test="${elderly.role == 6}">
                        <td class="td-status"><span class="label label-success radius">登录中</span></td>
                    </c:if>
                    <c:if test="${elderly.role == 7}">
                        <td class="td-status"><span class="label label-success radius">未登录</span></td>
                    </c:if>

                    <td class="td-manage"><%--<a style="text-decoration:none" onClick="admin_stop(this,'10001')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a> --%><a
                            title="编辑" href="javascript:;"
                            onclick="admin_edit('用户编辑','/user-edit',${elderly.id},'750','450')" class="ml-5"
                            style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除"
                                                                                                     href="javascript:;"
                                                                                                     onclick="admin_del(this,${elderly.id})"
                                                                                                     class="ml-5"
