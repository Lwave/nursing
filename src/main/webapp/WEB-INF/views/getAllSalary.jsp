...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 员工工资管理 <span class="c-gray en">&gt;</span> 员工工资管理 <span
...
<div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20"><span class="l"><a href="javascript:;" onclick="datadel()"
                                                               class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>  <a class="btn btn-primary radius" onclick="product_add('添加员工工资','/showInsertView','750','450')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加信息</a></span><span class="r">共有数据：<strong
            style="color: #5bc0de;">1</strong> 条</span>
    </div>
    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover table-sort">
            ...
            <tbody>
            <c:forEach items="${all}" var="all" varStatus="index">
                <tr class="text-c">
                    <td><input name="delete" type="checkbox" value="${all.id}"></td>
                    <td>${index.count}</td>
                    <td>${all.month.month}</td>
                    <td>${all.user.username}</td>
                    <td>${all.user.phone}</td>
                    <td>${all.user.idcard}</td>
                    <td>${all.user.salarycard}</td>
                    <td>${all.factsalary}</td>
                    <td>${all.performance}</td>
                    <td>${all.deductsalary}</td>
                    <td>${all.deductsituation}</td>
                    <td>${all.factsalary+all.performance+all.deductsalary}</td>
                    <td class="td-manage">
                        <a
                                style="text-decoration:none" class="ml-5"
                                onClick="product_edit('状态编辑','${pageContext.request.contextPath}/editShowEverySalary',${all.id},'750','600')"
                                href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
                        <a
                                style="text-decoration:none" class="ml-5" onClick="product_del(this,${all.id})"
...
<script type="text/javascript">
    ...
    function product_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            $.ajax({
                type: 'POST',
                url: '${pageContext.request.contextPath}/delSalary?id=' + id,
                dataType: 'json',
                success: function (data) {
                    $(obj).parents("tr").remove();
                    layer.msg('已删除!', {icon: 1, time: 1700});
                },
                error: function (data) {
